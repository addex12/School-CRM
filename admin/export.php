...
function exportPDF($survey, $fields, $responses, $response_data): never {
    ...
    // PDF content
    $html = '<h1>' . htmlspecialchars(string: $survey['title']) . '</h1>';
    $html .= '<p>' . htmlspecialchars(string: $survey['description']) . '</p>';
    $html .= '<p><strong>Total Responses:</strong> ' . count(value: $responses) . '</p>';
    $html .= '<hr>';
    
    foreach ($responses as $response) {
        $html .= '<h2>Response from ' . htmlspecialchars(string: $response['username']) . '</h2>';
        $html .= '<p><strong>Role:</strong> ' . htmlspecialchars(string: $response['role']) . '</p>';
        ...
        foreach ($response_data[$response['id']] as $data) {
            $html .= '<tr>';
            $html .= '<td>' . htmlspecialchars(string: $data['field_label']) . '</td>';
            $html .= '<td>' . nl2br(string: htmlspecialchars(string: $data['field_value'])) . '</td>';
            $html .= '</tr>';
        }
    ...
    header(header: 'Content-Disposition: attachment; filename="survey_' . $survey['id'] . '_results.pdf"');
